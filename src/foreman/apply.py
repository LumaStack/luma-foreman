...
def _adapter(bundles: list[Bundle], project_root: Path) -> str:
    ...
    **It points at the project ring and gets out of the way.** Everything it
    ...
    if always_on:
        lines += [
            "**These are in force here and arrive with this file.** Each one "
...
def _entrypoint(bundles: list[Bundle], project_root: Path) -> str:
    """The project ring: what this repository knows, and what fires each part.

    **Names and reasons, never bodies.** A reader arrives here knowing nothing
    and leaves knowing what exists and how to reach it — which is the whole job
    of a ring, and the reason its cost is one line per item rather than one
    document.

    Workflows are absent deliberately. They are carried to this harness as
    skills, whose names and descriptions are already loaded at session start,
    and **an adapter must not render what its harness already carries.** Their
    reachability is unaffected: the closing line points at them.
    """
    lines = [
        ...
        "repository. **Open one when the work matches its line, and not "
        "before.**",
        "",
    ]
    ...
    for bundle in bundles:
        ...
        # class.** Existence is cheap and content is expensive, so the ring
        # names these and delivers almost none of them. A policy that matches
        # nothing lands in on-demand, and a rule nobody can see governs nothing
        # — listing only the advertised ones would turn a cost saving into the
        # silent absence this whole design exists to end.
        #
        # Two deliberate exceptions. **Background** is left out on the format's
        # own reasoning (§10.7): it does not act, and it is reached *through*
        # the things that do. **Workflows** are left out because this harness
        # already loads every skill's name and description at startup, and an
        # adapter must not render what its harness already carries — 8,451
        # characters of this file were a second copy of exactly that. They stay
        # reachable by name, which the closing line says.
        shown = [d for d in bundle.docs
                 if d.type != "workflow"
                 and (d.klass == "advertised"
                      or (d.klass == "on-demand" and d.type == "policy"))]
        if not shown:
            lines += [f"Everything here arrives with `CLAUDE.md`. In `{home}/`.", ""]
            continue
    ...
    lines += [
        ...
        "invoked by name — they are not listed here because this harness "
        "already loads their names and descriptions. Everything under "
        "`.luma/bundles/` is a copy: change it upstream and take it again, "
        "never in place.",
        "",
    ]
    return "\n".join(lines)
