...
- `Edit(~/.config/luma/**)` and `Edit(~/.local/share/luma/**)` in
  ...
  and the gate. **One entry per directory covers every luma tool**, because they
  nest under the organization: no wildcard to rely on, nothing to widen when a
  second tool arrives. That matters because this rule fails open — a pattern
  matching nothing produces no error, and a rule needing no glob support cannot
  fail that way.
...
echo '{"tool_name":"Bash","cwd":"'"$PWD"'","tool_input":{"command":"sudo reboot"},"permission_mode":"default"}' \
  | ~/.local/share/luma/luma-foreman/permission-gate.sh
```
