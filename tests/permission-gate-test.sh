...
#
#   Bash policy. The hook resolves a per-project policy from $LUMA_FOREMAN_HOME
...
#
# The whole run is hermetic: LUMA_FOREMAN_HOME points into a temp dir, so the
# real ~/.config/luma/foreman never affects the result and is never written to.
#
# Expectations:
...
# hook reads a file the CLI never writes. On macOS /tmp -> /private/tmp
...
tb ask  'cp /tmp/evil ~/.config/luma/foreman/permission-gate.sh' bypassPermissions
tb ask  'chmod 000 ~/.config/luma/foreman/permission-gate.sh'
