#!/bin/sh
# IMPORTANT: Installed copy — the source of truth is luma-foreman.
# Reinstall with `luma-foreman policy install` rather than editing this file;
# an edit here is silently replaced on the next install and, more to the point,
# this file decides what an agent is allowed to do.
#
...
#      from ~/.config/luma/foreman. Every gated command class (ssh, curl, sudo, ...)
...
#
#      Every key has a default, so a machine with no config files at all is
#      still correctly gated. Configuration changes what you are asked about;
#      it is never what makes the gate exist.
#
...
#
# This script lives in the SAME directory as the policy it reads, and that is
# deliberate: one `Edit(~/.config/luma/**)` deny rule then protects both the
# rulebook and the thing that enforces it. A gate an agent can rewrite is not
# a gate.
#
# See docs/claude-permission-policy.md for how this interacts with the native
...
# Canonicalize before slugging. The CLI computes its slug from `pwd -P`, and
...
matches() {
  case $1 in
    ...
    downloads)
      ...
      ;;
    policy_write)
      # Writes to anything that decides permissions — the policy files AND this
      # gate script, which lives in the same directory precisely so that one
      # deny rule covers both. Without this the agent can hand itself any
      # permission it likes by editing its own rulebook, or by editing the
      # thing that reads the rulebook.
      #
      # The native Edit(~/.config/luma/**) deny in settings.json covers the file
      # tools; this covers Bash.
      case $cmd in *luma*|*foreman*) ;; *) return 1 ;; esac
      # The CLI's own mutating subcommands. Keep this list in step with the
      # dispatch table in libexec/policy: anything that writes belongs here.
      # The reads — show, list, keys, projects, path — stay ungated on purpose.
      printf '%s' "$cmd" | grep -Eq \
        '(^|[^[:alnum:]_.-])luma-foreman[[:space:]]+policy[[:space:]]+(-[^[:space:]]+[[:space:]]+)*(set|unset|reset|edit|allow|ask|deny|install)([[:space:]]|$)' && return 0
      # Any shell-level write aimed at the policy directory. Reading is fine.
      printf '%s' "$cmd" | grep -Eq '(\.config/luma|LUMA_FOREMAN_HOME|permission-gate\.sh)' || return 1
