...
- **The help is sectioned after `gh`, at the top level and in all three nouns.** A tagline, `USAGE`, commands grouped and ordered by how often they are reached rather than alphabetically, `FLAGS`, `EXAMPLES`, `NOTES`, `EXIT CODES`. Verbs are listed bare under their heading rather than each row repeating the noun it already sits under. **"Check a project against the baseline and report shortfalls" is gone** — the word appeared in exactly two sentences in the whole repository and named nothing: no concept, no file, no config. It read as more specific than it was and sent a reader looking for something that does not exist. `inspect` now says it checks what a repository declares against what is actually here, which is what its five rules do.
  *`NOTES` keeps what terseness would delete, on the test that phrasing failed*: a line stays if a reader can act on it. `get`, `apply`, `inspect` and `init` still print the older shape — only `inspect`'s opening line changed, because that is where the phantom noun lived.
- **`bundle list` says what each bundle is doing here, how it reaches an agent, and how many skills it holds.** A mark per row — `●` wired and working, `◐` here but not reaching an agent, `⊘` deliberately turned off, `○` not here — then the posture derived from the bundle's own `matches` (`eager`, `offered`, `standby`) and a skill count. Three of the marks sit on a fullness continuum; **`⊘` deliberately does not**, because being turned off is not a degree of health but somebody's decision, and a glyph reading as *less working* would misdescribe it.
  **The same four marks mean the same four things in `catalog show`**, which reports the same bundles from the catalog's side. One function answers for both, so they cannot disagree — and they did: `catalog show` called a bundle whose directory had been deleted *taken and applied*, because it asked the manifest whether the bundle was held and never asked the disk. It also reported a bundle turned off as merely not applied.
  **`◐` is a residual rather than a list of conditions** — everything except deliberately-off falls into it, and *unapplied* and *drifted* are only today's two detectable failures. The mapping is written as a default rather than an enumeration so the failure direction is right: naming the bad states and calling everything else healthy reports a bundle broken in some new way as `●`, which is the reading that costs most. A third condition lands under `◐` without the marks being touched.
  **Only states actually present get a legend line**, each naming the command that resolves that state — a key explaining marks nothing on screen is in is a key people stop reading. The legend keys on the cause rather than the mark, since the causes under `◐` are fixed differently: offering `apply` to somebody whose copy is applied and edited is a remedy that does nothing. A cause with no entry still gets a line and a way in, because a row marked as not working with nothing explaining it is worse than a vague explanation. The old trailing `inspect --rule adoption` line is gone, because every state that earned it now names its own fix.
  *The skill count is what a bundle holds, not what is loaded* — the mark already says whether it is active — and it is a number rather than a marker because the number is the cost signal. Reading it walks every document, which measures at 25ms against this repository's 87; a tree that will not parse degrades to no posture and no count rather than failing the listing, since *what do I hold* is worth more than completeness and `inspect` is what reports an unparseable bundle.

- **A bundle written in this project is recorded in `MANIFEST.md`, so the manifest-driven commands can see it.** `bundle new` now writes a bare entry — the bundle line and nothing under it, because source, commit and checksum are custody facts and a bundle written here has no custody. The shape is the distinction (ADR-0011); there is no `local: true` flag restating what the lines already show. Until now `apply` and `inspect` saw local bundles because they walk disk, while `bundle list`, `show` and `set` read the manifest and did not — so `bundle list` said *nothing adopted* about a project `apply` had just written a bundle out of, and `bundle set <it> register nothing` was unreachable for the bundles most likely to want it.
  **`bundle new` is idempotent and never destructive — `init`'s contract**, replacing the refusal it shipped with. It adds whatever is missing and leaves every existing file exactly as it is, which makes it the claiming command too: run it on a bundle somebody wrote by hand and it supplies the missing half, reading that author's own version rather than the template's. A `BUNDLE.md` is still never overwritten — that file is what makes a directory a bundle — and the report says per file what happened, so the two halves are never confused. Its next steps now list only what the run actually left undone.
  *Three readers stop saying things that this makes untrue.* `bundle show` prints `written here — no catalog, and nothing to compare against` in place of blank `source` and `commit` rows, and labels the path `at` rather than `vendored`. `bundle list` counts bundles rather than *adopted* bundles, since one written here was never adopted. `bundle outdated` reports a bare entry as `written here` and counts it apart from both the current and the unanswered — a bundle with no upstream cannot be behind one, and calling that *no source on record* would have given every project holding a local bundle a permanent question mark.
...
- **`luma-foreman publish <bundle> <catalog>` offers a bundle written here to a catalog, and finishes the handover once it lands.** Publishing is a custody handover: the project ends up an adopter of its own bundle, so the bare manifest entry becomes a receipt and editing the bundle afterwards is drift rather than authorship. Foreman opens a pull request containing the bundle at its final path — namespace rewritten, `published:` stamped, index regenerated; the catalog's own pre-merge job is what judges it; a maintainer merges. **The curator gains nothing and stays read-only** — it never adopts, and a design where it materialized a submission would stop the pull request being what a maintainer approves.
  **Two operands, because where a bundle is going is not in its name.** `get` needs one ID since the namespace already says which catalog; a bundle written here has none to read, and the same bundle may go to more than one catalog, so the destination is required rather than inferred. A bare name means `local/` and the prefix is accepted; a full ID names whichever bundle is meant.
  **One idempotent command, and the only automatic transition is the merged one.** Run it whenever: it opens the request, reports one still under review, or takes the published bundle back and retires the local one. Everything else reports and waits for a person. A declined request changes nothing, and one that is no longer visible enumerates the possibilities rather than concluding — **a 404 is not proof of rejection**, since GitHub answers 404 for a private resource you cannot see, so it also covers a catalog moved, gone private, or a token expired. Asking again takes `--again`, so a maintainer is never sent a duplicate nobody decided to send. `--abandon` stops tracking a request and keeps the destination.
  *`--again` rather than `--force`*: `--force` overrides a guard protecting something, and asking a catalog twice defends nothing. *`bundle show` reads the state without advancing it*, so nobody has to run the thing that moves it to find out where it stands.

- **`luma-foreman remove <bundle>` drops a bundle this project holds.** The inverse of `get` and shaped like it — one bundle ID, this project only, finished when it returns. It removes the directory and the manifest entry; `apply` then unwires the generated skills and the index, which is why this is a command rather than a note telling somebody to use `rm`.
  **The guard keys on recoverability, not on where the bundle came from** — refuse when removing would lose work nothing else holds. One rule covering a bundle written here that is not committed and a vendored copy somebody edited, which generalises the refusal `get` already makes rather than adding a second differently-shaped guard beside it. Everything else is recoverable and the output names the literal command: `get` for a clean vendored copy, `git checkout HEAD --` for a committed local one. It exits 1 when something still cites the bundle, naming the files — `inspect` catches a dangling wikilink afterwards, and a bundle cited as a bare path in prose is what it does not catch.
...
- **`inspect --rule adoption` asserts that a namespaced entry is verifiable.** A bundle recorded under a catalog's namespace must carry a checksum and say where it came from. Without one, `state()` skips the drift comparison entirely — so the check passes silently, forever, which is the single failure the checksum exists to prevent. **The commit is deliberately not required**: a catalog that is not a git checkout has none to record, which `get` already reports as `(not a git checkout)` rather than treating as an error, and demanding it here would report a sanctioned adoption as broken. This is what lets a bundle keep its `local/` ID while a publication request is outstanding — before the merge it has not been published, and the request may yet be declined.

### Fixed

- **The permission gate no longer fails open under a second command name.** Its `policy_write` rule and its undo exemption both keyed on the literal string `luma-foreman`, so a `foreman` symlink — the shape `luma-backlog` already has as `backlog` — would have made `foreman agent-permissions deny <key>` match nothing and be permitted silently. That is a hole in the one rule whose whole job is stopping an agent editing the rules. **The exemption mattered more than the rule**: it is what keeps the undo reachable, so with `curl = deny` set it is what stops the gate refusing `agent-permissions reset curl` — under a name it did not know, the lockout it exists to prevent would have come back under the other spelling. Both now accept `(?:luma-)?foreman`, non-capturing and optional as a whole; the preceding `[^\w.\-]` still excludes `-`, so `acme-foreman` cannot match.
  **Takes effect only after `luma-foreman agent-permissions install`** — the installed gate carries its own copy of these modules, which is what makes it survive the checkout moving. `agent-permissions doctor` reports the staleness.
