...
- **`bundle list` says what each bundle is doing here, how it reaches an agent, and how many skills it holds.** A mark per row — `●` wired and working, `◐` here but not reaching an agent, `⊘` deliberately turned off, `○` not here — then the posture derived from the bundle's own `matches` (`eager`, `offered`, `standby`) and a skill count. Three of the marks sit on a fullness continuum; **`⊘` deliberately does not**, because being turned off is not a degree of health but somebody's decision, and a glyph reading as *less working* would misdescribe it.
  ...
  **Only states actually present get a legend line**, each naming the command that resolves that state — a key explaining marks nothing on screen is in is a key people stop reading. `◐` covers *unapplied* and *drifted* deliberately, since both mean *look at this*, but the legend keys on the cause rather than the mark: offering `apply` to somebody whose copy is applied and edited is a remedy that does nothing. The old trailing `inspect --rule adoption` line is gone, because every state that earned it now names its own fix.
