...
type: luma/idea
type_version: "0.1.0"
title: How committed knowledge reaches an agent working elsewhere
created: { by: human:luma-founder, at: 2026-08-09T00:00:00Z }
contributors: [human:luma-founder]
horizon: someday
scope: project
stage: draft
---

# How committed knowledge reaches an agent working elsewhere

A committed file is durable but inert. Machine-local agent memory was live but
fragile and keyed to one directory on one machine. Neither is sufficient alone,
and this trade is unresolved — it is the same question parked in
`luma-foreman/examples/`.
...
**Added at migration, not part of the original idea:** the durable half now has a
form. `.luma/` is committed, portable, and travels with a clone, which leaves
**liveness** as the half still open.
