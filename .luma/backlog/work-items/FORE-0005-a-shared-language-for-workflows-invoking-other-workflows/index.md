...
description: 'A workflow should be able to declare that another workflow or command runs before, during or after it, and every workflow and tool should read that the same way. Declared in frontmatter under invokes: with a slug, placed in the body by a reserved luma-invoke: marker, with four levels, absence handling, a noop entry for documentation examples, and conditions written as prose introduced by when rather than as machine-evaluable expressions. A four-part admission test governs which target kinds are allowed; workflow ships first and concept is deferred for failing three of the four. The design settled over one conversation on 2026-08-21 and is entirely unbuilt — nothing declares an invocation and nothing reads one.'
modified: {by: 'agent:claude-opus-5/luma-backlog', at: '2026-09-24T19:42:36Z'}
---
...
here, is the same shape: the format supplies `stage`, this repository
...
**This is the caller's half only.** [[work-items/FORE-0006-a-bundle-exposes-named-routines-not-a-single-entry-point]] raises the other one —
whether a bundle exposes several named callables rather than the single
`entry_point` the `bundle:` target assumes here.

## Related work

- Born from the idea it replaces: [`workflows-invoking-workflows`](../../ideas/workflows-invoking-workflows.md)
