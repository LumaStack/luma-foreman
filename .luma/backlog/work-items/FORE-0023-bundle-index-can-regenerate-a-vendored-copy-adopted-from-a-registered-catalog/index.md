...
description: '`src/foreman/bundle_index.py:181`: `src/foreman/get.py:222`, writing the receipt: `tests/bundle-index-test.sh` builds its local case as `org/local` — it exercises the shape rule, that an entry with no custody sublines is a bundle written here. It never builds a name-indirect receipt, so the one arrangement that breaks this has no fixture.'
modified: {by: 'agent:claude-opus-5/luma-backlog', at: '2026-09-24T19:43:40Z'}
---
...
- Born from the idea it replaces: [`bundle-index-misreads-a-registered-catalog-copy`](../../ideas/bundle-index-misreads-a-registered-catalog-copy.md)
