...
kind: idea
stage: draft
created: {by: 'human:luma-founder', at: '2026-09-08T00:00:00'}
description: Nothing checks that an idea's `horizon` is one of those three, that `archived` parses as a date, or that a required field is present at all. `inspect --rule bundles` catches shallower things — frontmatter with no `type`, an unquoted wikilink, a template carrying live frontmatter — and stops there.
modified: {by: 'agent:claude-opus-5/luma-backlog', at: '2026-09-24T19:43:46Z'}
---
...
checks — the thing [[work-items/FORE-0040-verification-beyond-inspect-compliance-mandates-and-rot]] calls the rot with no
referent.

**It is also the missing half of
[[work-items/FORE-0064-frontmatter-needs-a-version-and-a-type-definition-needs-migrations]].** A version on a
...
**[[work-items/FORE-0017-nothing-checks-that-a-citation-resolves]] is the sibling gap.** That one
...
- Born from the idea it replaces: [`nothing-validates-frontmatter-against-its-type`](../../ideas/nothing-validates-frontmatter-against-its-type.md)
