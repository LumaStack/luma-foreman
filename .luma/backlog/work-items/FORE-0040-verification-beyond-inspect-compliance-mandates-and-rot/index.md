...
description: 'So the split is the design: build the mechanical half as `inspect` rules, and treat the rest as something a person or an agent finds by reading. A review sweep is one answer to the second half, which is worth noticing given one is running. Any of the above needs fields a tool can evaluate, so this is upstream of the rest of the entry rather than beside it.'
...
- **Mandates handed down from an organization**, probably through the catalog.
  ...
  [[work-items/FORE-0031-distribution-foreman-installs-by-clone-and-symlink]].
...
- Born from the idea it replaces: [`verification-beyond-inspect`](../../ideas/verification-beyond-inspect.md)
