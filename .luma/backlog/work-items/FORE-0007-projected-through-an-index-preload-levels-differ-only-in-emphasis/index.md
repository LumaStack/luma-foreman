...
kind: idea
stage: draft
created: {by: 'agent:claude-opus-5', at: '2026-08-23T00:00:00'}
description: 'The obvious stronger reading is that `mandatory` should mean the content is in context, so `apply` should paste the document into `CLAUDE.md` rather than linking it. Three things argue against it, and the first is the one that matters: routers — conditional loading, which is where the second reading leads. bundle-routines — a named routine is a per-call loading path, which would make this question moot by letting the caller choose rather than the author. `conditional-preload` in `luma-leader` — the same field from the format''s side.'
...
[[work-items/FORE-0049-routers-conditional-decision-logic-for-what-gets-loaded]] — conditional loading, which is where the second reading leads.
[[work-items/FORE-0006-a-bundle-exposes-named-routines-not-a-single-entry-point]] — a named routine is a per-call loading path, which would
...
- Born from the idea it replaces: [`preload-levels-collapse-into-emphasis`](../../ideas/preload-levels-collapse-into-emphasis.md)
