...
description: 'The underscore was collision avoidance from when types might have lived at project roots. They now live only inside bundles, where the author controls every sibling, so the fear is obsolete. The prefix''s other reading — machinery, not reading material — was never load-bearing: `templates/` gets the identical tooling exemption with no prefix.'
...
[[work-items/FORE-0063-retire-the-migration-tolerances-once-the-estate-has-re-adopted]] — one re-adopt wave can carry both.
...
[[plans/migrate-estate-to-type-definitions]], executed in full — kept in `plans/`
for now as a reference, past its own prune-when-done rule, at the
maintainer's request.

## Related work

- Born from the idea it replaces: [`rename-types-to-type-definitions`](../../ideas/rename-types-to-type-definitions.md)
