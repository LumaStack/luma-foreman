...
kind: idea
stage: draft
created: {by: 'human:luma-founder', at: '2026-08-28T00:00:00'}
description: 'So the two halves separate cleanly, and the split is worth stating as a rule: Rings stay a browsing surface. `/load-bundle` and `/list-bundles` already cover them and need no hook. So `topic:` becomes mechanically triggerable by keyword, not reliably matched. Better than nothing and much worse than the other three.'
...
- Born from the idea it replaces: [`hook-delivery`](../../ideas/hook-delivery.md)
