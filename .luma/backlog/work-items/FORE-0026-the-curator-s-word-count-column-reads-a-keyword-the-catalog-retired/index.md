...
kind: idea
stage: draft
created: {by: 'agent:claude-opus-5', at: '2026-09-06T00:00:00'}
description: The README calls that column "the number this tool exists to produce" — what a bundle costs an adopter in every session, the asymmetry a publisher-side tool exists to expose because an adopter cannot see it before adopting. That is the failure mode the curator's own commit history argues against — a check that stops finding things cannot be told from one that passes. Here the tool goes further and reassures.
...
[[work-items/FORE-0063-retire-the-migration-tolerances-once-the-estate-has-re-adopted]].
...
- Born from the idea it replaces: [`curator-word-count-reads-a-retired-keyword`](../../ideas/curator-word-count-reads-a-retired-keyword.md)
