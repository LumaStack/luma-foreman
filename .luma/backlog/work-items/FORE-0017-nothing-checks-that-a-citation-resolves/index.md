...
description: A document in this repository can cite a bundle, a policy, another record or an idea, and nothing verifies that the thing it names exists. A citation that resolves to nothing throws no error, changes no behaviour, and leaves the document looking exactly as authoritative as one that is correct.
...
- **`[[wikilinks]]`** — resolves to a tracked file somewhere it could plausibly
  ...
  idea links `[[work-items/FORE-0018-report-what-an-adoption-costs-before-it-loads]]`.
...
- Born from the idea it replaces: [`nothing-checks-that-a-citation-resolves`](../../ideas/nothing-checks-that-a-citation-resolves.md)
