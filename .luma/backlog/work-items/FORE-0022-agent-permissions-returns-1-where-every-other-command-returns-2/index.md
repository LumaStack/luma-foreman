...
description: '`src/foreman/agent_permissions/commands.py`: Every other command''s `_err` returns 2 and has a separate `_refuse` returning 1. This one has no `_refuse` and no third code at all. It also means the one command a script is most likely to wrap is the one whose exit codes say least.'
...
- Born from the idea it replaces: [`agent-permissions-exit-codes-diverge`](../../ideas/agent-permissions-exit-codes-diverge.md)
