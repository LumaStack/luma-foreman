...
description: It also cuts against the curator's own stated purpose. A catalog is meant never to be in a corrupt state; a pin nobody watches means the checker's fixes do not reach the one catalog that wired it as a gate. Found while building `publish`, and recorded until now only in an appendix to `.luma/backlog/plans/bundle-publish.md`.
...
- Born from the idea it replaces: [`catalog-ci-pins-a-stale-curator`](../../ideas/catalog-ci-pins-a-stale-curator.md)
