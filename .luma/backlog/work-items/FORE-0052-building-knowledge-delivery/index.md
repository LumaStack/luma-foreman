...
description: '`apply` writes the project ring to `.luma/bundles/entry-point.md`. The managed block in `CLAUDE.md` shrinks to an adapter that points at it. Measurable the moment it lands, and it needs nothing else decided. Today every document declaring `matches: always`, in any bundle, is imported into `CLAUDE.md` and sits in every session. Under the target only the project ring''s `always` gets that seat.'
...
- Born from the idea it replaces: [`knowledge-delivery`](../../ideas/knowledge-delivery.md)
