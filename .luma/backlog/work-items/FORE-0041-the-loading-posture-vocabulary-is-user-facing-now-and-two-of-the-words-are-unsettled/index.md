...
description: So this should be decided on which words are clearest, not on what they cost. Neither word is obviously right. `standby` suggests waiting to be activated, which overstates — nothing is watching for a moment to load it. `on-demand` suggests a mechanism, which also overstates, since the demand is somebody naming it. Worth weighing alongside by request, which is what the generated project index already calls this section, and unlisted, which describes what is actually true of it.
...
- Born from the idea it replaces: [`bundle-list-shows-loading-posture`](../../ideas/bundle-list-shows-loading-posture.md)
