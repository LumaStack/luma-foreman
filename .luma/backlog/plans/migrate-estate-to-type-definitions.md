...
> **Executed in full, 2026-09-20.** Every repository below conforms — the gate
> query returns zero `_types/` directories and zero typed documents missing
> `type_version` across all eight repos — and the migration tolerances this
> plan gated were retired (#160). Kept past its own prune-when-done rule at
> the maintainer's request, as a reference while the follow-on decisions
> (the linter, the ruleset bundles) are still drawing on it; pruning it later
